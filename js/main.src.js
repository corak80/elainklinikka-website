...
const translations = {
  ...
  'article.grainfree.fda.text': {
    fi: 'FDA tutki satoja raportteja dilatoivasta kardiomyopatiasta (DCM) — sydänsairaudesta, jossa sydänlihas heikkenee ja sydän laajenee. Huolestuttavaa oli, että DCM:ää esiintyi roduilla, jotka eivät normaalisti ole alttiita sille, kuten kultaisillanoutajilla, labradoreilla ja sekarotuisilla koirilla. Tutkimuksessa 91 % epäillyistä ruokavalioista oli viljattomia ja 93 % sisälsi herneitä tai linssejä pääainesosina. Vaikka suoraa syy-yhteyttä ei lopullisesti todistettu, monet koirat toipuivat, kun ruokavalio vaihdettiin viljapohjaiseen — mikä on merkittävä havainto.',
  ...
  'article.grainfree.advice.text': {
    fi: 'Älä valitse viljatonta ruokaa ilman lääketieteellistä syytä. Vältä ruokia, joissa herneet, linssit tai kikherneet ovat pääainesosina. Valitse ruoka tunnetuilta valmistajilta, jotka työllistävät eläinravitsemuksen asiantuntijoita ja toteuttavat ravintoon liittyviä tutkimuksia. Jos epäilet ruoka-aineallergiaa, ota yhteyttä eläinlääkäriin — oikea diagnoosi tehdään eliminaatiodieetillä, ei vaihtamalla satunnaisesti ruokaa. Jos lemmikkisi syö tällä hetkellä viljatonta ruokaa, keskustele eläinlääkärisi kanssa vaihtoehdoista.',
  ...
  'article.periovive.evidence.text': {
    fi: 'Hyaluronihapon tehosta parodontiittisairauksien hoidossa on runsaasti tutkimusnäyttöä. Koirilla tehdyt tutkimukset osoittivat 89 %:sen luun uudelleenmuodostumisen hoidetuissa kudosvaurioissa, verrattuna 39 %:iin kontrolliryhmässä. Ientaskujen syvyyden väheneminen ja kliininen kiinnityskudosten paraneminen olivat tilastollisesti merkittäviä. Hyaluronihapon tehoa tukevat yli 5 koirille tehtyä tutkimusta ja yli 100 ihmisille tehtyä tutkimusta parodontiittisairauksien hoidossa.',
    sv: 'Det finns omfattande forskningsbevis för hyaluronsyrans effekt vid behandling av parodontal sjukdom. I studier på hund modeller visade HA 89 % bennybildning i behandlade vävnadsdefekter, jämfört med 39 % i kontrollgruppen. Minskning av tandköttsfickornas djup och förbättrad klinisk fästning var statistiskt signifikanta. Effekten av hyaluronsyra stöds av över 5 hundstudier och över 100 humanstudier inom parodontal behandling.',
  ...
  'article.food.intro': {
    fi: 'Ruoka-aine allergia on yksi yleisimmistä kutinaa aiheuttavista sairauksista koirilla ja kissoilla. Tutkimusten mukaan 9–40 % kutisevista koirista ja 12–21 % kutisevista kissoista kärsii ruoka-aine allergiasta. Monet omistajat ajattelevat viljojen olevan ongelma, mutta tutkimukset osoittavat selvästi: yleisimmät allergeenit ovat eläinproteiineja — nauta (34 %), maito (17 %) ja kana (15 %). Oikea diagnoosi vaatii eliminaatiodieetin, ja me autamme siinä.',
  ...
  'article.kidney.treatment.text': {
    fi: 'Munuaissairautta ei voi parantaa, mutta etenemistä voidaan hidastaa merkittävästi. Hoidon kulmakiviä ovat: munuaisruokavalio (rajoitettu proteiini ja fosfori — tutkitusti pidentää elinaikaa), lääkehoito, nestehoito (ihonalainen tai suonensisäinen), verenpainelääkitys ja pahoinvointilääkitys. Anemiaa hoidetaan tarvittaessa erytropoietiinilla. Säännöllinen seuranta (verikokeet, verenpaine, virtsakoe) on välttämätöntä hoidon sopeuttamiseksi. Klinikallamme seuraamme munuaispotilaita yksilöllisellä aikataululla.',
  ...
  'article.pyometra.symptoms.text': {
    fi: 'Pyometra kehittyy tyypillisesti 2–8 viikkoa kiiman jälkeen. Avoimessa pyometrassa kohdunkaulasta vuotaa märkäistä, usein pahanhajuista eritettä — tämä on selkein merkki. Suljetussa pyometrassa eritettä ei tule ulos, mikä tekee taudista erityisen vaarallisen, koska oireet ovat epämääräisempiä. Muita oireita ovat: lisääntynyt juominen ja virtsaaminen, ruokahaluttomuus, väsymys ja apaattisuus, oksentelu, turvonnut tai kipeä vatsa ja kuume (tai alilämpö vakavissa tapauksissa). Jos steriloimaton narttu on väsynyt ja juo paljon kiiman jälkeen, hakeudu eläinlääkäriin heti.',
  ...
  'article.pyometra.treatment.text': {
    fi: 'Pyometran ainoa hoito on kohdunpoisto (ovariohysterektomia) eli sterilisaatio. Leikkaus tehdään kiireellisesti, usein samana päivänä. Ennen leikkausta potilas stabiloidaan nestehoidolla ja aloitetaan suonensisäinen antibioottihoito. Leikkauksessa poistetaan sekä infektoitunut kohtu että munasarjat, mikä estää taudin uusiutumisen. Lääkinnällistä hoitoa ei suositella, koska tauti uusiutuu jopa 75 %:ssa tapauksista. Leikkaushoidolla ennuste on hyvä — yli 90 % potilaista toipuu leikkauksen jälkeen.',
  ...
  'article.hedgehog.intro': {
    fi: 'Euroopan siili (Erinaceus europaeus) on yksi rakastetuimmista villieläimistämme — ja yksi nopeimmin katoavista lajeista. Lokakuussa 2024 IUCN nosti siilin uhanalaisuusluokituksen luokasta "elinvoimainen" luokkaan "silmälläpidettävä", koska kannat ovat romahtaneet yli 30 % kymmenessä vuodessa monissa Euroopan maissa. Suomessa siili elää levinneisyytensä pohjoisrajalla, mikä tekee kannastamme erityisen haavoittuvan. Tässä artikkelissa käymme läpi siilin tilanteen, uhat ja sen, miten jokainen voi auttaa.',
  ...
  'article.unerupted.intro': {
    fi: 'Puhkeamaton hammas on hammas, joka ei ole noussut normaalisti suuonteloon vaan jäänyt leukaluun sisään. Tila on yllättävän yleinen erityisesti koirilla — noin 5–7 % koirista kärsii siitä, ja lyhytkuonoisilla roduilla osuus on vielä suurempi. Puhkeamattoman hampaan ympärille voi kehittyä kysta, nestettä täynnä oleva rakenne, joka kasvaa hitaasti ja tuhoaa leukaluuta. Ilman hammasröntgenkuvausta tilanne jää havaitsematta, kunnes vahinko on jo merkittävä. Klinikallamme suoritamme hammasröntgenkuvaukset ja puhkeamattomien hampaiden kirurgiset poistot.',
  ...
  'article.deworming.title': {
    fi: 'Kuinka usein koira ja kissa pitää madottaa? Näin ajoitat sisäloishäädön oikein',
    sv: 'Hur ofta ska hunden och katten avmaskas? En riskbaserad avmaskningsguide',
    en: 'How Often Should You Deworm Your Dog and Cat? A Risk-Based Deworming Guide'
  },
  'article.deworming.intro': {
    fi: 'Suomessa madotuksesta ajatellaan nykyään toisin kuin ennen. Rutiininomaisesta, kaikille samanlaisesta ja monta kertaa vuodessa toistetusta madotuksesta on siirrytty riskiperusteiseen, ulostenäytteeseen nojaavaan sisäloishäätöön. Ajatus on yksinkertainen: emme lääkitse turhaan, vaan hoidamme oikeaan aikaan ja oikeasta syystä. Terve, matalan riskin aikuinen lemmikki ei tarvitse säännöllistä, useita kertoja vuodessa toistuvaa madotusta, kun taas pennut, kissanpennut ja riskiryhmiin kuuluvat eläimet hyötyvät säännöllisestä häädöstä. Paras mittari hoidon tarpeelle on ulostenäyte, ei kalenteri.',
    sv: 'I Finland har vi gått ifrån den gamla vanan att avmaska alla djur rutinmässigt flera gånger om året. I dag följer vi ett riskbaserat tänkande som stöder sig på avföringsprov: vi medicinerar inte i onödan, utan behandlar vid rätt tidpunkt. En vuxen familjehund eller innekatt med låg risk behöver sällan regelbunden avmaskning, medan valpar, kattungar och djur i riskgrupp nog behöver avmaskas med jämna mellanrum. På den här sidan går vi lugnt igenom vad just ditt djur behöver.',
    en: 'In Finland, deworming is approached differently today than before. We have moved away from routine, one-size-fits-all deworming repeated several times a year, towards risk-based parasite control that relies on a fecal test: we don\'t medicate unnecessarily, but treat at the right time and for the right reason. A healthy, low-risk adult pet does not need regular deworming several times a year, whereas puppies, kittens and pets in risk groups benefit from regular treatment. The best measure of need is a fecal test, not the calendar.'
  },
  'article.deworming.dogs.title': {
    fi: 'Koiran madotus: pennusta aikuiseksi',
    sv: 'Avmaskning av hund – valp och vuxen',
    en: 'Deworming Dogs: From Puppy to Adult'
  },
  'article.deworming.dogs.text': {
    fi: '<strong>Pennut:</strong> ensimmäinen madotus tehdään jo <strong>2 viikon iässä</strong>, sillä suolinkaiset voivat siirtyä pentuihin jo kohdussa ja imetyksen kautta. Tämän jälkeen häätö toistetaan <strong>kahden viikon välein vieroitukseen asti</strong> ja sen jälkeen <strong>kerran kuukaudessa 6 kuukauden ikään saakka</strong>. Emä madotetaan aina samaan aikaan pentueen kanssa.<br><br><strong>Aikuiset:</strong> tavalliselle, matalan riskin perhekoiralle riittää usein madotus <strong>1–2 kertaa vuodessa</strong>, mieluiten ulostenäytteen perusteella. Suuremman riskin koirilla – kuten metsästys-, matkustus- ja kennelkoirilla, vapaasti ulkoilevilla sekä ulosteita tai maata syövillä – sisäloishäätö voidaan tehdä <strong>jopa 4 kertaa vuodessa</strong>. Metsästyskaudella heisimatoja vastaan voidaan tarvittaessa lääkitä <strong>kuukausittain</strong>.',
    sv: '<strong>Valpar:</strong> Den första avmaskningen görs redan vid <strong>2 veckors ålder</strong>, därefter <strong>varannan vecka fram till avvänjningen</strong> och sedan <strong>en gång i månaden tills valpen är 6 månader</strong>. Tiken avmaskas samtidigt som kullen, eftersom parasiterna kan överföras redan i livmodern och via mjölken.<br><br><strong>Vuxen hund:</strong> För en familjehund med låg risk räcker det oftast med avmaskning <strong>1–2 gånger om året</strong>, helst utgående från ett avföringsprov. Hundar med högre risk – jakt-, rese- och kennelhundar, hundar som rör sig fritt utomhus eller som äter avföring och jord – kan behöva avmaskas <strong>upp till 4 gånger om året</strong>. Under jaktsäsongen ges vid behov en behandling <strong>en gång i månaden mot bandmask</strong>.',
    en: '<strong>Puppies:</strong> the first deworming is done already at <strong>2 weeks of age</strong>, because roundworms can pass to puppies in the womb and through nursing. It is then repeated <strong>every 2 weeks until weaning</strong> and after that <strong>once a month until 6 months of age</strong>. The mother is always dewormed at the same time as the litter.<br><br><strong>Adults:</strong> for an ordinary, low-risk family dog, deworming <strong>1–2 times a year</strong> is often enough, preferably based on a fecal test. For higher-risk dogs – such as hunting, travelling and kennel dogs, dogs that roam freely outdoors, and those that eat feces or soil – deworming can be done <strong>up to 4 times a year</strong>. During the hunting season, treatment against tapeworm may be given <strong>monthly</strong> if needed.'
  },
  'article.deworming.cats.title': {
    fi: 'Kissan madotus: kissanpennusta aikuiseksi',
    sv: 'Avmaskning av katt – kattunge och vuxen',
    en: 'Deworming Cats: From Kitten to Adult'
  },
  'article.deworming.cats.text': {
    fi: '<strong>Kissanpennut:</strong> ensimmäinen madotus tehdään <strong>3 viikon iässä</strong>, minkä jälkeen häätö toistetaan <strong>kahden viikon välein, kunnes vieroituksesta on kulunut kaksi viikkoa</strong>. Tämän jälkeen madotus tehdään <strong>kuukausittain 6 kuukauden ikään saakka</strong>.<br><br><strong>Aikuiset:</strong> sisäkissalle riittää yleensä madotus <strong>1–2 kertaa vuodessa</strong>, mieluiten ulostenäytteen perusteella. Ulkoileva tai jyrsijöitä pyydystävä kissa madotetaan <strong>4 kertaa vuodessa</strong>, ja aktiivisesti saalistava kissa <strong>kuukausittain</strong> – hiiret ja muut jyrsijät levittävät heisimatoja.',
    sv: '<strong>Kattungar:</strong> Den första avmaskningen görs vid <strong>3 veckors ålder</strong>, därefter <strong>varannan vecka fram till två veckor efter avvänjningen</strong> och sedan <strong>en gång i månaden tills kattungen är 6 månader</strong>.<br><br><strong>Vuxen katt:</strong> En innekatt klarar sig oftast med avmaskning <strong>1–2 gånger om året</strong>, helst utgående från ett avföringsprov. En katt som rör sig ute och fångar gnagare hör till riskgruppen och avmaskas <strong>4 gånger om året</strong>, medan en katt som jagar aktivt behöver behandling <strong>en gång i månaden</strong>, eftersom gnagare sprider bandmask.',
    en: '<strong>Kittens:</strong> the first deworming is done at <strong>3 weeks of age</strong>, after which it is repeated <strong>every 2 weeks until two weeks after weaning</strong>. After that, deworming is done <strong>monthly until 6 months of age</strong>.<br><br><strong>Adults:</strong> an indoor cat usually manages with deworming <strong>1–2 times a year</strong>, preferably based on a fecal test. A cat that goes outdoors and catches rodents belongs to the risk group and is dewormed <strong>4 times a year</strong>, while a cat that hunts actively needs treatment <strong>monthly</strong>, since mice and other rodents spread tapeworm.'
  },
  'article.deworming.risk.title': {
    fi: 'Kenen lemmikki tarvitsee madotusta – ja milloin ulostenäyte riittää?',
    sv: 'Riskgrupper och avföringsprov',
    en: 'Who Needs Deworming — and When Is a Fecal Test Enough?'
  },
  'article.deworming.risk.text': {
    fi: 'Madotuksen tarve arvioidaan aina <strong>yksilöllisesti lemmikin elämäntavan mukaan</strong>. Riski on suurempi, jos eläin on <strong>alle 1-vuotias</strong>, ulkoilee vapaana, saalistaa tai pyydystää saaliseläimiä tai syö ulostetta, raakaa lihaa tai jyrsijöitä.<br><br>Turhaa lääkitystä kannattaa välttää. Matalan riskin aikuiselle lemmikille suosittelemme ensin <strong>ulostenäytettä</strong>: jos näyte on puhdas, madotusta ei tarvita lainkaan. Aikuisille riittää yleensä <strong>ulostenäyte kerran vuodessa</strong> – näin lääkitään oikeaan aikaan eikä varmuuden vuoksi.',
    sv: 'Behovet av avmaskning bedöms alltid <strong>individuellt utgående från djurets levnadssätt</strong>. Till <strong>högre risk</strong> hör djur som är <strong>under 1 år</strong>, som rör sig <strong>fritt utomhus</strong>, som <strong>jagar och fångar byten</strong> eller som äter <strong>avföring, rått kött eller gnagare</strong>.<br><br>För att undvika onödig medicinering rekommenderar vi att man för en vuxen hund eller katt med låg risk först lämnar in ett <strong>avföringsprov</strong> – är provet rent behövs ingen avmaskning. För vuxna djur räcker det oftast med ett <strong>avföringsprov en gång om året</strong>.',
    en: 'The need for deworming is always assessed <strong>individually, based on the pet\'s lifestyle</strong>. The risk is higher if the animal is <strong>under 1 year old</strong>, roams freely outdoors, hunts or catches prey, or eats feces, raw meat or rodents.<br><br>Unnecessary medication is best avoided. For a low-risk adult pet we first recommend a <strong>fecal test</strong>: if the sample is clean, no deworming is needed at all. For adults, a <strong>fecal test once a year</strong> is usually enough – this way we treat at the right time, not just in case.'
  },
  'article.deworming.parasites.title': {
    fi: 'Yleisimmät suolistoloiset Suomessa',
    sv: 'Vanliga inälvsparasiter i Finland',
    en: 'The Most Common Intestinal Parasites in Finland'
  },
  'article.deworming.parasites.text': {
    fi: '<strong>Suolinkaiset (Toxocara):</strong> yleisin loinen pennuilla ja kissanpennuilla. Suolinkaiset ovat <strong>zoonoottisia</strong> eli voivat tarttua myös ihmiseen, mikä on tärkeää huomioida erityisesti lapsiperheissä.<br><br><strong>Heisimadot:</strong> esiintyvät pääasiassa eläimillä, jotka syövät jyrsijöitä tai raakaa lihaa.<br><br><strong>Giardia:</strong> alkueläin, joka aiheuttaa ripulia ja leviää ulosteen saastuttaman veden ja ympäristön kautta.<br><br><strong>Ekinokokki:</strong> tätä heisimatoa <strong>ei esiinny Suomessa</strong>, ja tuontimääräykset on laadittu juuri siksi, että tilanne pysyy näin.',
    sv: '<strong>Spolmask (Toxocara):</strong> den vanligaste parasiten hos valpar och kattungar. Spolmasken är <strong>zoonotisk</strong>, alltså kan den smitta människor, vilket är särskilt viktigt att komma ihåg i familjer med barn.<br><br><strong>Bandmask:</strong> förekommer främst hos djur som äter gnagare eller rått kött.<br><br><strong>Giardia:</strong> en encellig tarmparasit som orsakar diarré och sprids via vatten och miljö som förorenats av avföring.<br><br><strong>Echinococcus (rävens dvärgbandmask):</strong> förekommer <strong>inte i Finland</strong>, och det är importbestämmelserna som håller den borta.',
    en: '<strong>Roundworms (Toxocara):</strong> the most common parasite in puppies and kittens. Roundworms are <strong>zoonotic</strong>, meaning they can also infect humans, which is especially important to keep in mind in families with children.<br><br><strong>Tapeworms:</strong> occur mainly in animals that eat rodents or raw meat.<br><br><strong>Giardia:</strong> a protozoan that causes diarrhea and spreads through water and environments contaminated with feces.<br><br><strong>Echinococcus:</strong> this tapeworm <strong>does not occur in Finland</strong>, and the import requirements exist precisely to keep it that way.'
  },
  'article.deworming.travel.title': {
    fi: 'Matkustaminen ja lakisääteinen ekinokokkihäätö',
    sv: 'Resor med hund – lagstadgad avmaskning',
    en: 'Travel and Mandatory Echinococcus Treatment'
  },
  'article.deworming.travel.text': {
    fi: 'Kun koira palaa Suomeen ulkomailta, sille on lain mukaan tehtävä <strong>ekinokokkihäätö</strong>. Häätöön käytetään <strong>pratsikvantelia</strong> (tai yhtä tehokasta valmistetta), ja se annetaan <strong>1–5 vuorokautta (24–120 tuntia) ennen Suomeen saapumista</strong>.<br><br>Usein matkustaville koirille on oma <strong>28 vuorokauden sääntönsä</strong>: annetaan kaksi aloitushäätöä, minkä jälkeen häätö toistetaan <strong>enintään 28 vuorokauden välein</strong>, ja viimeinen annos annetaan Suomessa matkan jälkeen. Matkustamisen ja EU-lemmikkipassin yksityiskohdista kerromme tarkemmin <a href="/articles/rokotukset.html">rokotusartikkelistamme</a>.',
    sv: '<strong>Reser du utomlands med hunden?</strong> När en hund återvänder till Finland krävs enligt lag en behandling mot <strong>echinococcus (rävens dvärgbandmask)</strong>. Behandlingen ges med <strong>prazikvantel</strong> (eller ett lika effektivt preparat) <strong>1–5 dygn (24–120 timmar) före ankomsten till Finland</strong>.<br><br>För dig som reser ofta gäller <strong>28-dygnsregeln</strong>: två inledande behandlingar, därefter upprepning med <strong>högst 28 dygns mellanrum</strong>, och en sista dos i Finland efter resan. Fullständig information om resor och EU-pass hittar du i <a href="/sv/artiklar/vaccinationsguide.html">vår vaccinationsartikel</a>.',
    en: 'When a dog returns to Finland from abroad, <strong>echinococcus treatment is required by law</strong>. The treatment uses <strong>praziquantel</strong> (or an equally effective product), given <strong>1–5 days (24–120 hours) before arrival in Finland</strong>.<br><br>Frequently travelling dogs have their own <strong>28-day rule</strong>: two initial treatments, after which treatment is repeated at <strong>intervals of no more than 28 days</strong>, with the final dose given in Finland after the trip. You can read more about travel requirements and the EU pet passport in our <a href="/en/articles/vaccinations-guide.html">vaccination article</a>.'
  },
  'article.deworming.cta': {
    fi: 'Autamme mielellämme arvioimaan juuri sinun lemmikkisi madotustarpeen ilman turhaa lääkitystä. Varaa aika ulostenäytteeseen tai sisäloishäätöön Eläinklinikka Saareen – yhdessä varmistamme, että lemmikkiäsi hoidetaan oikeaan aikaan.',
    sv: 'Är du osäker på om just din hund eller katt behöver avmaskas? Boka gärna ett avföringsprov eller en avmaskning hos oss på Eläinklinikka Saari, så hjälper vi dig att hitta rätt rytm för just ditt djur.',
    en: 'Not sure whether your dog or cat needs deworming? Book a fecal test or a deworming appointment with us at Eläinklinikka Saari, and we\'ll help you find the right rhythm for your own pet.'
  },
